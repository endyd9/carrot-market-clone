...
interface ReviewWithUsers extends Review {
  createdBy: User;
}
...
const Reviews = () => {
  const { data } = useSWR<ReviewsResponse>(
    typeof window === "undefined" ? null : "/api/users/reviews"
  );
  return (
    <>
  ...
  return (
    <>
      <div className="flex items-center space-x-3">
        {user?.avatar ? (
          <img
            src={`https://imagedelivery.net/e47gKtH1bqlCtb8hOWHyxQ/${user?.avatar}/avatar`}
  ...
  return (
    <Layout title={`나의 캐럿`} seoTitle="나의 캐럿" hasTabBar>
      <div className="py-10 px-4">
        <Suspense fallback={`프로필 로딩`}>
          <MiniProfile />
        </Suspense>
        <div className=" mt-10 flex justify-around">
          <Link href={`profile/sold`} className="flex flex-col items-center">
            <div className="w-14 h-14 mb-1 text-white bg-orange-500 rounded-full flex items-center justify-center">
              <svg
                className="w-6 h-6"
                fill="none"
                stroke="currentColor"
